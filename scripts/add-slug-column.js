...
async function addSlugColumn() {
  try {
    ...
    if (hasSlug) {
      ...
      const indexes = await turso.execute('PRAGMA index_list(products)');
      const hasSlugIndex = indexes.rows.some(row => row.name === 'idx_slug');
      ...
      if (!hasSlugIndex) {
        ...
        console.log('✓ Индекс для slug уже существует');
      }
    } else {
